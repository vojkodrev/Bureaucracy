...
import {
    CalendarRange, ChevronRight, FileDown, FileText, Landmark, PackageSearch,
    Plus, ReceiptText, Search, Upload, Users, type LucideIcon,
} from 'lucide-react'
...
const sections: Section[] = [
    ...
    { key: 'invoices', label: 'Invoices', icon: FileText, searchPath: '/invoices/search', entityPath: '/invoice', entityLabel: 'Invoice' },
    { key: 'bank-statements', label: 'Bank statements', icon: Landmark, searchPath: '/bank-statements/search', entityPath: '/bank-statement', entityLabel: 'Bank statement', additionalItem: { path: '/bank-statements/import', label: 'Import', icon: Upload } },
]
...
export default function LayoutSidebar({ pathname, sidebarOpen }: { pathname: string; sidebarOpen: boolean }) {
    const navigate = useNavigate()
    const activeSection = sections.find((section) => sectionIsActive(section, pathname))?.key ?? null
    ...
    return (
        <Sidebar collapsible="icon">
            <SidebarHeader><SidebarMenu><SidebarMenuItem>
                <SidebarMenuButton size="lg" tooltip="Bureaucracy" render={<NavLink to="/invoices/search" />}>
                    <img src="/favicon.svg" alt="Bureaucracy logo" className="size-8 rounded-lg" />
                ...
                {sections.map((section) => {
                    ...
                    return (
                        <Collapsible.Root
                            ...
                            onOpenChange={(open) => {
                                if (!sidebarOpen) return navigate(section.searchPath)
                                setOpenMenu(open ? section.key : null)
                            }}
                            render={<SidebarMenuItem />}
                        >
                            <Collapsible.Trigger render={<SidebarMenuButton
                                isActive={sectionIsActive(section, pathname)}
                                tooltip={section.label}
                                className="cursor-pointer data-open:[&>svg:last-child]:rotate-90"
                                render={!sidebarOpen ? <NavLink to={section.searchPath} /> : undefined}
                            />}>
                                <Icon /><span>{section.label}</span><ChevronRight className="ml-auto transition-transform" />
                            </Collapsible.Trigger>
                            <Collapsible.Panel render={<SidebarMenuSub />}>
                                ...
                                {section.additionalItem && (() => {
                                    ...
                                    return <SidebarMenuSubItem><SidebarMenuSubButton
                                        isActive={pathname === section.additionalItem.path}
                                        render={<NavLink to={section.additionalItem.path} />}
                                    ><ItemIcon /><span>{section.additionalItem.label}</span></SidebarMenuSubButton></SidebarMenuSubItem>
                                })()}
                            </Collapsible.Panel>
                        </Collapsible.Root>
                    )
                })}
                <SidebarMenuItem><SidebarMenuButton isActive={pathname === '/business-years'} tooltip="Business years" render={<NavLink to="/business-years" />}>
                    <CalendarRange /><span>Business years</span>
                </SidebarMenuButton></SidebarMenuItem>
                <SidebarMenuItem><SidebarMenuButton isActive={pathname === '/export'} tooltip="Export data" render={<NavLink to="/export" />}>
                    <FileDown /><span>Export data</span>
                </SidebarMenuButton></SidebarMenuItem>
