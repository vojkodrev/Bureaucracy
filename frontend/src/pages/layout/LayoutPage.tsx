...
function breadcrumbLabel(pathname: string) {
    ...
    for (const [path, label] of entities) {
        ...
        if (pathname.startsWith(`${path}/`)) {
            ...
            try {
                return `${label} ${decodeURIComponent(encodedValue)}`;
            } catch {
...
export default function LayoutPage() {
    ...
    const [openedBusinessYear, setOpenedBusinessYear] = useState(
        getSelectedBusinessYear,
    );
    const [selectedBusinessYear, setSelectedBusinessYear] = useState(
        getSelectedBusinessYear,
    );
    ...
    useEffect(() => {
        const checkBusinessYear = () =>
            setSelectedBusinessYear(getSelectedBusinessYear());
    ...
    useEffect(() => {
        ...
        if (sortBy && (direction === "asc" || direction === "desc")) {
            details.push(
                `Sort: ${sortLabels[sortBy] ?? sortBy} ${direction === "asc" ? "A" : "D"}`,
            );
        }
        document.title = ["Bureaucracy", businessYear, titleLabel, ...details]
            .filter(Boolean)
            .join(" - ");
    ...
    return (
        <TooltipProvider>
            ...
            <AlertDialog
                open={businessYearChanged}
                onOpenChange={() => undefined}
            >
                <AlertDialogContent>
                    <AlertDialogHeader>
                        <AlertDialogTitle>
                            Business year changed
                        </AlertDialogTitle>
                        <AlertDialogDescription>
                            This page was opened for business year{" "}
                            {businessYear ?? openedBusinessYear}. Restore the
                            selected business year to{" "}
                            {businessYear ?? openedBusinessYear} to continue.
                        </AlertDialogDescription>
                    </AlertDialogHeader>
                </AlertDialogContent>
            </AlertDialog>
